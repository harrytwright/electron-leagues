...
Status: agreed design, not implemented. Baseline: branch `claude/league-members-database-3jt90g`
at `321fd9f` (Polish desktop task dialogs and location toolbar). This replaces the earlier
export-driven draft after a full design review; the decisions below are settled.
...
- One MBD all-bowlers export and one per-league export (personal data redacted) to fix the
  column mapping and become test fixtures.
- Confirmation of the MBD gender values as exported.
- What the POS scanner reads, to confirm Code 128.
