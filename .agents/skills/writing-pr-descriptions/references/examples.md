...
Three merged PostHog PRs, where this skill originated, as shipped and after all five passes. Read these when the rule in `SKILL.md` is clear but you want to see it applied end to end. The template sections they use differ from ours, but the passes apply unchanged.
...
The measurements behind these rules live in the upstream PostHog PR that introduced the skill, not in this repository's history. They are provenance for whoever edits this skill, not instructions for writing a body.
